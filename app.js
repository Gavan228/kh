...
var i32s = function(x) {
    return String.fromCharCode.apply(null, new Uint8Array([(x & 0xff000000) >> 24, (x & 0x00ff0000) >> 16, (x & 0x0000ff00) >> 8, (x & 0x000000ff)]));
}
var s32i = function(x) {
...
function updateBots() {
    ...
    bots.filter((e)=>ss == "" || e.username.toLowerCase().includes(ss)).forEach(function(entry, index) {
        ...
        [i++, entry.username, "<span class='label label-" + zz.type + "'>" + zz.message + "</span>", entry.autoplay ? "On" : "Off", entry.place, entry.score, "<button class=\"btn btn-primary managebot\" bot=\"" + identifier + "\" style=\"margin-right: 10px;\">Manage</button><button class=\"btn btn-danger deletebot\" bot=\"" + identifier + "\">Delete</button>"].forEach(function(e, i) {
    ...
    $("#managerform").submit(function(e) {
        ...
        if (bo.autoplay != newplay) {
            sendPacket(hackop.BOTUPD, i32s(identifier) + "\x00");
        }
        bo.autoplay = newplay;
    ...
    $(".deletebot").click(function() {
        ...
        if (bots[identifier].status == botstates.CONNECTED)
            sendPacket(hackop.BOTUPD, i32s(identifier) + "\x01");
